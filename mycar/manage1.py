...
from donkeycar.parts.record_monitor import Monitor
from donkeycar.parts.voice import monitor
from donkeycar.parts.Playsound import playsound
import socket
...
print(ip)
skt.close()
os.system("ssh -fCNR 8888:"+ip+":8887 sice@10.33.32.6")
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None, camera_type='single', meta=[] ):
    ...
    V.add(ctr, 
          ...
          outputs=['user/angle', 'user/throttle', 'user_mode', 'recording'],
          threaded=True)
    ...
    #IMU
        ...
        inputs=['cam/image_array','road/sel']
    else:
        inputs=['cam/image_array']
    ...
    if model_path:
        ...
        def update(self):
            while True:
                self.pilot_state = self.run()
                print(self.pilot_state)
    ...
    V.add(MySwitch(), inputs=[], outputs=['run_mypilot'], threaded=True)'''
    playsound1=playsound()
    V.add(playsound1,inputs=['angle','throttle'],outputs=[],threaded=True)
    class Vvoice:
        def __init__(self):
            self.control = 'user'
            self.sel = 1
    ...
    V.add(Vvoice(), inputs=[], outputs=['user/mode', 'road/sel'], threaded=True)
    ...
    '''class Sound:
        ...
        def run_threaded(self, angle, throttle):
            ...
            if angle is not None and throttle is not None:
                ...
                t = threading.Thread(target=bofang, args=(angle,throttle,))
                t.start()
    ...
    V.add(Sound(), inputs=['angle', 'throttle'], outputs=[], threaded=True)'''
    ...
    if type(ctr) is LocalWebController:
        ...
        os.system('mplayer %s' % 'music/jzcg.mp3')
...
if __name__ == '__main__':
    args = docopt(__doc__)
    cfg = dk.load_config()
    os.system('mplayer %s' % 'music/td.mp3')
    ...
    if args['drive']:
        ...
        drive(cfg, model_path=bpath, use_joystick=args['--js'], model_type=model_type, camera_type=camera_type,
            meta=args['--meta'])
